...
Write-Host "[5/11] Creating Storage Account..." -ForegroundColor Yellow
...
Write-Host "[6/11] Creating Storage Container..." -ForegroundColor Yellow
...
Write-Host "[7/11] Getting Storage Account Key..." -ForegroundColor Yellow
...
Write-Host "[8/11] Creating Container Registry..." -ForegroundColor Yellow
...
Write-Host "[9/11] Creating App Service Plan..." -ForegroundColor Yellow
...
Write-Host "[10/11] Creating Django Web App..." -ForegroundColor Yellow
...
# Step 11: Celery Worker App
Write-Host "[11/11] Creating Celery Worker App..." -ForegroundColor Yellow
az webapp create --resource-group tf-neepco-rg --plan tf-neepco-plan --name tf-neepco-celery --deployment-container-image-name tfneepcoacr.azurecr.io/talentflow:latest --output none
if ($LASTEXITCODE -ne 0) { Write-Host "FAILED: Celery Worker App" -ForegroundColor Red; exit 1 }
Write-Host "  Celery Worker App created" -ForegroundColor Green
