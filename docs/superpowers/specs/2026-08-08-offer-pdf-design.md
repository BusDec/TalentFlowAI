...
- **Employee number**: `f"{advt.post.advertisement.advt_number[:6]}-{post.post_code}-{seq:04d}"` where seq = count of joined applications for that post + 1 (deterministic; employee_number is not unique-constrained, keep simple; documented). (Amended: the original draft referenced a nonexistent `company_code` field; the advt-number prefix is implementable and stable per post.)
